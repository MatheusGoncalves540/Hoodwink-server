...
func ValidateConnection(w http.ResponseWriter, r *http.Request, jwtService *services.JWTService, upgrader websocket.Upgrader, rdb *redis.Client, ctx context.Context) (*websocket.Conn, jwt.MapClaims, error) {
	...
	if registered && connectedToRoomID != roomID {
		...
		return nil, nil, fmt.Errorf("player já está conectado em uma sala: %s", connectedToRoomID)
		...
		if player == nil {
			...
			return nil, nil, fmt.Errorf("player %s não encontrado na sala que deveria estar conectado", playerID)
		}
		if player.Connected {
			utils.SendError(w, "Player já está conectado na sala", http.StatusForbidden)
			return nil, nil, fmt.Errorf("player %s já está conectado na sala", playerID)
		}
	}
